  ...
  <div class="app-container p-4 md:p-6">
    ...
    <div class="mb-6">
      ...
      <el-button
          ...
          @click="handleQueryId"
          v-has-permi="['function:auth:query']"
      >
        高级设置
      </el-button>
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="24" :md="8" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  ...
                  <el-tooltip content="用于标识当前柜子的编号，将显示在控制台首页，仅支持数字，最长11位">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <el-input
                    v-model="settings.keyNumber"
                    placeholder="请输入序列号"
                    class="w-full transition-all"
                    @update:modelValue="handleInput('keyNumber')"
                    :class="{ 'error-input': errors.keyNumber }"
                    disabled
                />
              ...
              <el-col :span="24" :md="8" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  柜子名称
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="用于标识当前柜子的名称，将显示在控制台首页">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <el-input
                    v-model="settings.cabinetNamePrefix"
                    placeholder="请输入柜子名称"
                    class="w-full transition-all"
                    @update:modelValue="handleInput('cabinetNamePrefix')"
                    :class="{ 'error-input': errors.cabinetNamePrefix }"
                />
              ...
              <el-col :span="24" :md="8" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  地理位置
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="钥匙柜的物理位置">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <el-input
                    v-model="settings.cabinetNameSuffix"
                    placeholder="请输入柜子名称"
                    class="w-full transition-all"
                    @update:modelValue="handleInput('cabinetNameSuffix')"
                    :class="{ 'error-input': errors.cabinetNameSuffix }"
                />
            ...
            <el-row :gutter="20" class="mt-6">
              ...
              <el-col :span="24" :md="12" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  管理密码
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="用于登录管理后台的密码，建议定期更换">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <el-input
                    ...
                    placeholder="请输入管理密码"
                    class="w-full transition-all"
                    @update:modelValue="handleInput('adminPassword')"
                    :class="{ 'error-input': errors.adminPassword }"
                    show-password
                >
                  <template #suffix>
                    <el-icon
                        :class="showPassword ? 'cursor-pointer text-gray-400 hover:text-primary transition-colors' : 'cursor-pointer text-gray-400 hover:text-primary transition-colors'"
              ...
              <el-col :span="24" :md="12" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  柜子容量
                  <el-tooltip content="容量由硬件决定，不可修改">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
            ...
            <el-row :gutter="20" class="mt-6">
              ...
              <el-col :span="24" :md="12" class="space-y-1">
                <label class="block text-sm font-medium text-gray-700">
                  定时重启
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="开启后将按设定时间自动重启设备，提高稳定性">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <div class="flex items-center space-x-3">
                  <el-select
                      v-model="settings.scheduledReboot"
                      placeholder="请选择状态"
                      clearable
                      style="width: 240px"
                      @change="handleSelectChange('scheduledReboot')"
                      :class="{ 'error-input': errors.scheduledReboot }"
                  >
                    <el-option
              ...
              <!-- 重启时间 -->
              <el-col v-if="settings.scheduledReboot === 'enable'" :span="24" :md="12" class="space-y-1">
                <label class="block text-sm font-medium text-gray-700">
                  重启时间
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="设备将在每天的该时间自动重启，请选择设备使用率较低的时段">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <el-time-picker
                    v-model="settings.rebootTime"
                    format="HH:mm"
                    value-format="HH:mm"
                    placeholder="选择重启时间"
                    :disabled="!settings.scheduledReboot"
                    class="no-prefix-icon"
                    @change="handleInput('rebootTime')"
                />
      ...
      <el-col :span="24">
        ...
        <el-card shadow="hover" class="transition-all duration-300 border-0 mb-6">
          ...
          <div class="p-1 md:p-4 space-y-6">
            <el-row :gutter="20">
              ...
              <el-col :span="24" :md="12" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  钥匙申请
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="开启后用户钥匙借用需要申请">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <div class="flex items-center space-x-3">
                  <el-select
                      v-model="settings.keyRequest"
                      placeholder="请选择状态"
                      clearable
                      style="width: 240px"
                      @change="handleSelectChange('keyRequest')"
                      :class="{ 'error-input': errors.keyRequest }"
                  >
                    <el-option
                        v-for="dict in dict.type.key_cabinet_status"
                        :key="dict.value"
              ...
              <!-- 借复审 -->
              <el-col v-if="settings.keyRequest === 'enable'" :span="24" :md="12" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  借用审核
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="开启后钥匙借用需要管理员审核通过">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <div class="flex items-center space-x-3">
                  <el-select
                      v-model="settings.borrowReview"
                      placeholder="请选择状态"
                      clearable
                      style="width: 240px"
                      @change="handleSelectChange('borrowReview')"
                      :class="{ 'error-input': errors.borrowReview }"
                  >
                    <el-option
                        v-for="dict in dict.type.key_cabinet_status"
                        :key="dict.value"
            ...
            <el-row :gutter="20" class="mt-6">
              ...
              <el-col :span="24" :md="12" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  归还周期限制
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="开启后限制钥匙最长借用时间">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <div class="flex items-center space-x-3">
                  <el-select
                      v-model="settings.returnCycle"
                      placeholder="请选择状态"
                      clearable
                      style="width: 240px"
                      @change="handleSelectChange('returnCycle')"
                      :class="{ 'error-input': errors.returnCycle }"
                  >
                    <el-option
                        v-for="dict in dict.type.key_cabinet_status"
                        :key="dict.value"
              ...
              <!-- 归还时间 -->
              <el-col v-if="settings.returnCycle === 'enable'" :span="24" :md="12" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  最长借用时间
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="设置钥匙最长可借用的天数">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <el-select
                    v-model="settings.returnTime"
                    placeholder="选择天数"
                    class="w-full"
                    @change="handleSelectChange('returnTime')"
                    :class="{ 'error-input': errors.returnTime }"
                >
            ...
            <el-row v-if="settings.returnCycle === 'enable'" :gutter="20" class="mt-6">
              <!-- 超时提醒 -->
              <el-col :span="24" :md="12" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  超时提醒
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="当钥匙借用超时时发送提醒通知">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <div class="flex items-center space-x-3">
                  <el-select
                      v-model="settings.timeoutReminder"
                      placeholder="请选择状态"
                      clearable
                      style="width: 240px"
                      @change="handleSelectChange('timeoutReminder')"
                      :class="{ 'error-input': errors.timeoutReminder }"
                  >
                    <el-option
                        v-for="dict in dict.type.key_cabinet_status"
                        :key="dict.value"
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="24" :md="12" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  自动返回主页
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="长时间无操作后自动返回首页的时间设置">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <el-select
                    v-model="settings.returnToHome"
                    placeholder="选择时间"
                    class="w-full"
                    @change="handleSelectChange('returnToHome')"
                    :class="{ 'error-input': errors.returnToHome }"
                >
              ...
              <el-col :span="24" :md="12" class="space-y-2">
                <label class="block text-sm font-medium text-gray-700">
                  语音播报
                  <span class="text-red-500 ml-1">*</span>
                  <el-tooltip content="操作时是否进行语音提示">
                    <el-icon class="text-gray-400 ml-2 cursor-help hover:text-primary transition-colors"><InfoFilled /></el-icon>
                  </el-tooltip>
                </label>
                <div class="flex items-center space-x-3">
                  <el-select
                      v-model="settings.voiceAnnouncement"
                      placeholder="请选择状态"
                      clearable
                      style="width: 240px"
                  ...
                  <p v-if="errors.voiceAnnouncement" class="mt-1 text-sm text-red-500">{{ errors.voiceAnnouncement }}</p>
                </div>
              </el-col>
            </el-row>
          </div>
...
import { ElButton, ElCard, ElCol, ElRow, ElInput, ElTooltip, ElSelect, ElOption, ElTimePicker, ElIcon } from 'element-plus'
import { RefreshRight, Check, Tools, Key, Setting, InfoFilled, View, Hide } from '@element-plus/icons-vue'
...
function getKeyId() {
  ...
  getKeyConfiguration(id).then(res => {
...
function saveSettings() {
  ...
  saving.value = true;
  updateKeyConfiguration(settings).then(res => {
    Object.assign(originalSettings, settings);
    window.$message.success('设置保存成功');
    Object.assign(settings, res.data);
    hasChanged.value = false;
    const obj = {path: "/function/keyConfiguration"};
...
// 重置表单
function resetForm() {
  window.$confirm('确定要恢复默认设置吗？当前修改将会丢失', '提示', {
